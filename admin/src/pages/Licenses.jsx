...
export default function Licenses() {
  const stats = [
    { label: 'Total Trips', val: '1,248', sub: '+12%', icon: FileText, color: 'text-blue-600' },
    { label: 'Total Distance', val: '65,432 km', sub: '-5%', icon: Navigation, color: 'text-emerald-600' },
    { label: 'SOS Alerts', val: '18', sub: '-10%', icon: ShieldAlert, color: 'text-amber-600' },
    { label: 'Rescues', val: '24', sub: '+25%', icon: LifeBuoy, color: 'text-purple-600' },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <h2 className="text-sm font-bold text-slate-900">Licenses & Fleet Analytics</h2>
          ...
          return (
            <div key={i} className="bg-white border border-slate-200/80 rounded-xl p-4 flex items-center justify-between shadow-xs">
              <div>
                ...
                <span className="text-[10px] font-bold text-emerald-600">{s.sub}</span>
              </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="bg-white border border-slate-200/80 rounded-xl p-5 shadow-xs space-y-3">
          <h3 className="text-xs font-bold text-slate-900">Trips Over Time</h3>
          <div className="h-40 flex items-end justify-between gap-2 pt-4 px-2">
            {[180, 240, 210, 280, 260, 310].map((v, idx) => (
              <div key={idx} className="flex-1 flex flex-col items-center gap-1.5">
                <div className="w-full bg-blue-100 rounded-t hover:bg-blue-200 transition" style={{ height: `${(v / 310) * 100}%` }}>
        ...
        <div className="bg-white border border-slate-200/80 rounded-xl p-5 shadow-xs space-y-3">
          <h3 className="text-xs font-bold text-slate-900">Top Districts by Boats</h3>
          <div className="space-y-2 text-xs">
            <div className="flex justify-between"><span>Ramanathapuram</span><span className="font-bold text-blue-600">35%</span></div>
            <div className="flex justify-between"><span>Thoothukudi</span><span className="font-bold text-emerald-600">25%</span></div>
            <div className="flex justify-between"><span>Nagapattinam</span><span className="font-bold text-purple-600">18%</span></div>
            <div className="flex justify-between"><span>Kanyakumari</span><span className="font-bold text-amber-600">12%</span></div>
        ...
        <div className="bg-white border border-slate-200/80 rounded-xl p-5 shadow-xs space-y-3">
          <h3 className="text-xs font-bold text-slate-900">Weather Alerts Breakdown</h3>
