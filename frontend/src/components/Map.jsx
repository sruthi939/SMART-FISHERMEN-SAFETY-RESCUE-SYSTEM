...
export default function Map({ coordinates = { lat: 9.9312, lng: 76.2673 }, title = 'Live Maritime Map' }) {
  return (
    <div className="relative w-full h-80 rounded-xl overflow-hidden border border-slate-800 bg-slate-950 flex flex-col justify-between p-4">
      ...
      {/* Simulated Radar Center Pin */}
      <div className="relative z-10 self-center flex flex-col items-center gap-2">
        <div className="w-12 h-12 rounded-full bg-cyan-500/10 border border-cyan-500/40 flex items-center justify-center animate-ping absolute" />
        <div className="w-10 h-10 rounded-full bg-slate-900 border-2 border-cyan-400 text-cyan-400 flex items-center justify-center shadow-lg shadow-cyan-500/30">
          <Navigation size={18} className="transform rotate-45" />
        </div>
        <span className="bg-slate-900/90 text-cyan-300 text-[10px] font-mono px-2 py-0.5 rounded border border-cyan-500/30 shadow">
          VESSEL-104 ACTIVE
        </span>
      </div>
      ...
      <div className="relative z-10 flex items-center justify-between text-[11px] text-slate-400 bg-slate-900/80 px-3 py-1.5 rounded-lg border border-slate-800">
        ...
        <span className="text-emerald-400 font-bold">STATUS: SAFE</span>
      </div>
    </div>
  );
}
