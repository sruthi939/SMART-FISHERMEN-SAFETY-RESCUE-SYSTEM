...
export default function TripHistory() {
  ...
  const trips = [
    ...
    {
      id: '2',
      boatName: 'Blue Wave (Ramesh)',
      status: 'Completed',
      startTime: 'May 13, 2025 - 06:00 AM',
      endTime: 'May 13, 2025 - 04:30 PM',
      zone: 'Gulf of Mannar',
      crew: 3
    }
  ];
          ...
          <div key={trip.id} className="bg-white border border-slate-200/80 rounded-xl p-5 shadow-xs space-y-4">
            <div className="flex items-center justify-between border-b border-slate-100 pb-3">
              <div className="flex items-center gap-2.5">
                ...
                <strong className="text-slate-900 font-extrabold text-sm">{trip.boatName}</strong>
              </div>
              <span className={`px-2.5 py-0.5 rounded-full text-[10px] font-extrabold border ${
                trip.status === 'On Trip' ? 'bg-emerald-50 text-emerald-600 border-emerald-300' : 'bg-blue-50 text-blue-600 border-blue-300'
              }`}>
                {trip.status}
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-xs">
              <div>
                ...
                <strong className="text-slate-900 font-mono">{trip.startTime}</strong>
              </div>

              <div>
                <span className="text-slate-400 font-semibold block">
                  {trip.status === 'On Trip' ? 'Expected Return' : 'End Time'}
                </span>
                <strong className="text-slate-900 font-mono">{trip.expectedReturn || trip.endTime}</strong>
              </div>

              <div>
                <span className="text-slate-400 font-semibold block">Fishing Zone</span>
                <strong className="text-slate-900">{trip.zone}</strong>
              </div>

              <div>
                <span className="text-slate-400 font-semibold block">Crew Members</span>
                <strong className="text-slate-900 font-mono">{trip.crew}</strong>
              </div>
            </div>
